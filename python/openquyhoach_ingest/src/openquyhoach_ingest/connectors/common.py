...
def http_client(user_agent: str | None = None) -> httpx.Client:
    s = get_settings()
    return httpx.Client(
...
def get_json(url: str, params: dict | None = None, user_agent: str | None = None):
    """GET a JSON endpoint with SSRF validation. Returns parsed body."""
    check_url_allowed(url)
    with http_client(user_agent) as c:
        resp = c.get(url, params=params)
        resp.raise_for_status()
        return resp.json()


def get_xml(url: str, user_agent: str | None = None):
